  ...
  return (
    <div
      ...
      style={{
        ...
        '--accent-color': '#5865f2',
        '--border-color': '#202225',
        '--font-main-right': '"Hubot Sans", "Mona Sans", -apple-system, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif',
        '--font-main-left': '"Blimone", "Hubot Sans", "Mona Sans", -apple-system, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"',
      ...
      <div className="h-full w-full grid grid-rows-[auto_1fr_auto] text-[var(--text-color-primary)] bg-black/80 backdrop-blur-xl">
        ...
        <div className="flex-1 grid md:grid-cols-[1.2fr_1fr] grid-cols-1 overflow-y-auto relative">
          ...
          <main className="md:border-r border-b md:border-b-0 border-[var(--border-color)] py-14 px-10 md:px-16 flex flex-col transition-colors duration-300">
            <h2 className="text-xs font-medium text-[var(--text-color-secondary)] tracking-[0.12em] uppercase mb-16 relative after:content-[''] after:absolute after:bottom-[-1rem] after:left-0 after:w-[50px] after:h-[2px] after:bg-[var(--accent-color)] hover:after:w-[100px] after:transition-all after:duration-300">Main</h2>
            <nav>
              <ul className="group">
                {mainNav.map(item => (
                  <li key={item.label}>
                    <Link
                      ...
                      style={{ fontFamily: 'var(--font-main-left)', fontFeatureSettings: '"liga", "ss02"' }}
                    >
                      {item.label}
                    </Link>
                  </li>
          ...
          <aside className="py-14 px-10 md:px-16 transition-colors duration-300">
            <header className="flex justify-between items-center mb-8">
              <h2 className="text-xs font-medium text-[var(--text-color-secondary)] tracking-[0.12em] uppercase relative after:content-[''] after:absolute after:bottom-[-1rem] after:left-0 after:w-[50px] after:h-[2px] after:bg-[var(--accent-color)] hover:after:w-[100px] after:transition-all after:duration-300">More</h2>
              <Link href="#" className="text-xs text-[var(--text-color-secondary)] tracking-[0.08em] font-medium uppercase transition-colors hover:text-[var(--accent-color)]">See All &gt;</Link>
            </header>
            <nav>
              <ul className="relative group">
                {secondaryNav.map(topic => (
                  <li key={topic.label} className="relative after:content-[''] after:absolute after:bottom-0 after:left-[-2.5rem] md:after:left-[-4rem] after:right-[-2.5rem] md:after:right-[-4rem] after:h-[1px] after:bg-[var(--border-color)] last:after:hidden">
                    <Link
                      href={topic.href}
                      onClick={onClose}
